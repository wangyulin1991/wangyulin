 ...
 * @property mixed id
 */
class MessagePostModel extends Model
{
    ...
    {
        return $this->belongsToMany('MessageCategoryModel', 'message_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('MessageTagModel', 'message_tag_post', 'tag_id', 'post_id');
    }
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('message_tag_post')->where('post_id', $articleId)->column('tag_id');

            foreach ($keywords as $keyword) {
                $keyword = trim($keyword);
                if (!empty($keyword)) {
                    $findTag = $messageTagModel->where('name', $keyword)->find();
                    if (empty($findTag)) {
                        $tagId = $messageTagModel->insertGetId([
                            'name' => $keyword
                        ]);
                    } else {
                        $tagId = $findTag['id'];
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('message_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($data)) {
                Db::name('message_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('message_post')->where('id', $id)->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'message_post';
                ...
                $transStatus = false;
                try {
                    Db::name('message_post')->where('id', 'in', $ids)
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'message/Page/index', ['id' => $data['id']], 2, 5000);
