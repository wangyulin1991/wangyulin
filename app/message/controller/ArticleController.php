...
class ArticleController extends HomeBaseController
{
    ...
    public function index()
    {
        ...
        if (empty($categoryId)) {
            ...
            $category = $messageCategoryModel->where('id', $categoryId)->where('status', 1)->find();
        ...
        Db::name('message_post')->where('id', $articleId)->setInc('post_hits');
        Db::name('message')->where(['rec_user_id'=>$userId,'message_id'=>$articleId])->update(['status'=>1]);

        hook('message_before_assign_article', $article);
    ...
    public function doLike()
    {
        ...
        if ($canLike) {
            Db::name('message_post')->where('id', $articleId)->setInc('post_like');
