...
class PostService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        if (!empty($category)) {
            array_push($join, [
                '__MESSAGE_CATEGORY_POST__ b', 'a.id = b.post_id'
            ]);
        ...
        $articles        = $messagePostModel->alias('a')->field($field)
            ->join($join)
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $article = $messagePostModel->alias('post')->field('post.*')
                ->where($where)
            ...
            $join    = [
                ['__MESSAGE_CATEGORY_POST__ relation', 'post.id = relation.post_id']
            ];
            $article = $messagePostModel->alias('post')->field('post.*')
                ->join($join)
                ->where($where)
