...
class ApiService
{
    ...
    public static function articles($param)
    {
        ...
        if (!empty($categoryIds)) {
            ...
            array_push($join, ['__MESSAGE_CATEGORY_POST__ category_post', 'post.id = category_post.post_id']);
        ...
        $articles = $messagePostModel->alias('post')->field($field)
            ->join($join)
            ->where($where)
            ->where($paramWhere)
    ...
    public static function tagArticles($param)
    {
        ...
        if (empty($tagId)) {
            ...
            array_push($join, ['__MESSAGE_TAG_POST__ tag_post', 'post.id = tag_post.post_id']);
    ...
    public static function article($id)
    {
        ...
        return $messagePostModel->where($where)
    ...
    public static function category($id)
    {
        ...
        return $messageCategoryModel->where($where)->find();
    }
    ...
    public static function subCategories($categoryId, $field = '*')
    {
		...
		return $messageCategoryModel->field($field)->where($where)->order('list_order ASC')->select();
	}
    ...
    public static function allSubCategories($categoryId)
    {
        ...
        if ($categoryId !== 0) {
            $category = $messageCategoryModel->field('path')->where('id', $categoryId)->find();
        ...
        return $messageCategoryModel->where($where)->select();
    }
    ...
    public static function breadcrumb($categoryId, $withCurrent = false)
    {
        ...
        $path = $messageCategoryModel->where(['id' => $categoryId])->value('path');
        ...
        if (!empty($path)) {
            ...
            if (!empty($parents)) {
                $data = $messageCategoryModel->where('id', 'in', $parents)->order('path ASC')->select();
            }
        }
