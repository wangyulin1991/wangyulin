...
class ArticlesController extends RestBaseController
{
    ...
    public function __construct(MessagePostModel $postModel)
    {
        parent::__construct();
    ...
    public function read($id)
    {
        if (intval($id) === 0) {
            ...
            if (empty($data)) {
                ...
                $url = cmf_url('message/Article/index', ['id' => $id, 'cid' => $data['categories'][0]['id']], true, true);
    ...
    public function doLike()
    {
        ...
        $findLikeCount = $userLikeModel->where([
            'user_id'   => $userId,
            'object_id' => $articleId
        ])->where('table_name', 'message_post')->count();
        ...
        if (empty($findLikeCount)) {
            ...
            try {
                ...
                $userLikeModel->insert([
                    ...
                    'url'         => json_encode(['action' => 'message/Article/index', 'param' => ['id' => $articleId, 'cid' => $article['categories'][0]['id']]]),
                    'create_time' => time()
                ]);
                Db::commit();
            } catch (\Exception $e) {
        ...
        if (!empty($findLikeCount)) {
            ...
            try {
                ...
                $userLikeModel->where([
                    'user_id'   => $userId,
                    'object_id' => $articleId
                ])->where('table_name', 'message_post')->delete();
                Db::commit();
            } catch (\Exception $e) {
                Db::rollback();
                $this->error('取消点赞失败！');
    ...
    {
        $articleId  = $this->request->param('id', 0, 'intval');
        $categoryId = Db::name('message_category_post')->where('post_id', $articleId)->value('category_id');


        $articles = $this->postModel->alias('post')->join('__MESSAGE_CATEGORY_POST__ category_post', 'post.id=category_post.post_id')
