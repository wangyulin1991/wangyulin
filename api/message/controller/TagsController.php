...
use api\message\model\MessageTagModel;

class TagsController extends RestBaseController
{
    protected $tagModel;

    public function __construct(MessageTagModel $tagModel)
    {
        parent::__construct();
    ...
    public function articles($id)
    {
        if (intval($id) === 0) {
            ...
            $postModel = new MessagePostModel();

            unset($params['id']);

            $articles = $postModel->paramsFilter($params)->alias('post')
                ->join('__MESSAGE_TAG_POST__ tag_post', 'post.id = tag_post.post_id')
