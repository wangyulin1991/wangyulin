...
// +----------------------------------------------------------------------
namespace api\message\model;

use think\Db;
use api\common\model\CommonModel;

class MessagePostModel extends CommonModel
{
    //可查询字段
    protected $visible = [
    ...
    {
        return $this->belongsTo('api\message\model\UserModel', 'user_id');
    }
    ...
    {
        return $this->belongsTo('api\message\model\UserModel', 'user_id')->field('id,user_nickname');
    }
    ...
    {
        return $this->belongsToMany('api\message\model\MessageCategoryModel', 'message_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('api\message\model\MessageTagModel', 'message_tag_post', 'tag_id', 'post_id');
    }
    ...
    {
        return $this->hasOne('api\message\model\RecycleBinModel', 'object_id');
    }
        ...
        if (!empty($keywords) || !empty($names)) {
            if (!empty($names)) {
                ...
                if (!empty($shouldDeleteNames)) {
                    ...
                    $tagPosts   = DB::name('message_tag_post')->where('tag_id', 'in', $tagIds)
                    ...
                    DB::name('MessageTag')->delete($shouldDeleteTagIds);
                    DB::name('MessageTagPost')->delete($tagPostIds);
                }
            } else {
                $tagIdNames = DB::name('Message_tag')->where('name', 'in', $keywords)->column('name', 'id');
                if (!empty($tagIdNames)) {
                    $tagIds = array_keys($tagIdNames);
