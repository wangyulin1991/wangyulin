...
hello;
    ...
    public function checkShopkeeperLogin()
    {
        $userId = cmf_get_current_user_id();
        if (empty($userId)) {
            if ($this->request->isAjax()) {
                $this->error("您尚未登录", cmf_url("shopkeeper/Public/login"));
            } else {
                $this->redirect(cmf_url("shopkeeper/Public/login"));
            }
        }
    }
    ...
    protected function uploadImage($file){
        return $this->uploadFiles($file, $this->config);
    }
